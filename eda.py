...
plt.scatter(np.delete(length_prots, inds_to),
            np.delete(np.array(train_data["pH"]), inds_to), alpha=0.01)
plt.show()
...
# mh_tm
plt.scatter(np.delete(np.array(train_data["pH"]), ph_idiot[0]),
            np.delete(np.array(train_data["tm"]), ph_idiot[0]), alpha=0.01)
plt.show()
